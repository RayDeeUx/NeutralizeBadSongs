...
public:
	...
	void onSettingsButton(CCObject*) {
		if (m_title == "Refresh Replacement Songs Pool, \"Banned\" Artists + Song IDs") {
			if (Utils::refreshLists()) return FLAlertLayer::create("Success!", "You've <cg>successfully</c> refreshed your pool of replacement songs.", "Close")->show();
			return FLAlertLayer::create("Hmm...", "Something went <co>wrong</c> when trying to refresh your pool of replacement songs.", "Oof...")->show();
		}
